...
You can set or update these identifiers from **My Properties** > _select a property_ > <img src="../media/modify-property-icon.png" alt="pencil icon" style="height: 1em; vertical-align: middle;">.

## Internal identifier

The **Property Internal name** is used only by your team within the dashboard. It helps you and your staff recognize and refer to the property in a consistent way.
...
The **Property guest name** is displayed to guests in Alfred's chat and in other guest-facing touchpoints such as Booking.com, Airbnb, or direct host communications.
...
The **Guest access PIN** is a short numeric code (e.g., *4739*) used to verify the guest as your customer.  
It prevents unauthorized users from accessing Alfred and ensures that only real guests can interact with your property's assistant.

- Must be unique for each apartment  
- Shared manually with guests by the host  
...
When a guest contacts Alfred's phone number, they need to authenticate using both identifiers:

1. **Property guest name** — Alfred asks which apartment they are staying in. The guest must provide the exact Property guest name.
2. **Guest access PIN** — Alfred then asks for the PIN to verify their identity.

> **IMPORTANT** The Property guest name is **case sensitive**. Guests must enter it exactly as you've set it, including capital and lowercase letters. For example, if the name is "Apartment 2B", entering "apartment 2b" or "APARTMENT 2B" won't work.

If a guest enters the wrong PIN, they can try again. Make sure to share both the Property guest name and PIN clearly with your guests to avoid authentication issues.
